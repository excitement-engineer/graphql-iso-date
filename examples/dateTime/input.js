...
graphql(schema, `{ input(date: "2016-02-01T13:10") }`)
    .then(result => {
      console.log(`Example query { input(date: "2016-02-01T13:10") }: Input a valid date and the date in format YYYY-MM-DDThh:mm:ss.SSSZ`)
      console.log(result)
    })
    .catch(console.error)

graphql(schema, `{ input(date: "2015-02-25T22:61") }`)
    .then(result => {
      console.log(`Example query { input(date: "2015-02-25T22:61") }: Output an error when an invalid date is passed as input (an hour cannot have 61 minutes)`)
      console.log(result)
    })
    .catch(console.error)
